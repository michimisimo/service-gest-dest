...
exports.getDestinatariosActivos = async () => {
    const { data, error } = await supabase.from('destinatario').select('*').eq('activo', true);
...
exports.updateEstadoDestinatario = async (rut) => {
    const { data, error } = await supabase.from('destinatario').update({ 'activo': false }).eq('rut', rut);
    if (error) throw new Error(error.message);
    return data;
};